...
using namespace std;
    ...
    Double_t FitChiSquare(const double* para)
    {
        ...
        TFile      *Hist_FileName = new TFile("Spec_508keV_90deg_1-0kV_F90_0-45_0-85_Hist.root", "READ");
        
        //TH1F          *smoothed_hist_S1  = (TH1F*)Hist_FileName->Get("S1Spec_200_200");
        //TH1F          *smoothed_hist_S1  = (TH1F*)Hist_FileName->Get("S1Spec_100_200");
        //TH1F          *smoothed_hist_S1  = (TH1F*)Hist_FileName->Get("S1Spec_100_100");
...
int main(int argc, char **argv){
  ...
  min->SetFunction(f);
    ...
    //min->SetVariable(2,"Amp",                    variable[2], step[2]);
     ...
     /*   
    std::cout<<"Potential Output: "<<min->Minimize()<<std::endl;
    //min->Minimize();
    
    const double *Result = min->X();
    std::cout<<"MinValue is"<<min->MinValue()<<".  Detector Resolution: "<<Result[0]<<", PMT Resolution: "<<Result[1]<<", Amp: "<<Result[2]<<std::endl;
